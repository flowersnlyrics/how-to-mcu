...
6. If `x` divided by `y` is `>2` then print `2` five times. If `x`  isdivided by `y` is `<2` then print `2` four times. If it's equal to `2` print `2` two times, otherwise just print `"too many twos"`.
